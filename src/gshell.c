...
int main(int argc, const char* argv[]){
	...
	while (1) {
		printf("yash-shell> "); fflush(stdout);
	...
	while (cmd_tok != NULL) {
		...
		if (1 == pipeFound) {
			...
			if (0 == WEXITSTATUS(status2)){
				printf("[yash-shell: program terminated successfully]\n");
			} else {
				printf("[yash-shell: program terminated abnormally] [%d]\n", WEXITSTATUS(status2));
			}
		} else {
...
void get_date(void) {
	...
	if (rc) {
		perror("[yash-shell: program terminated abnormally]: [gettimeofday failed]\n");
	}
	t = tv.tv_sec;
	rv = ctime(&t);
	if (NULL == rv) {
		perror("[yash-shell: program terminated abnormally]: [ctime failed]\n");
	}
	printf("%s", rv);
	printf("[yash-shell: program terminated successfully]\n");
	fflush(stdout);
